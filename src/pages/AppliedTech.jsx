...
export default function AppliedTech() {
  const [sheets, setSheets] = useState([]);

  useEffect(() => {
    const fetchSheets = async () => {
      try {
        const data = await sheetService.getSheets('TECH');
        setSheets(data);
      } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-green-600 mb-4">AppliedTech Sheets</h1>
      <AddSheetForm onAdd={handleAddSheet} type="TECH" />
